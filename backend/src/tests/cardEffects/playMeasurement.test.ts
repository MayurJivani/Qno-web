...
test('Each player should receive correct hand and opponent hand', async () => {
    ...
    const [yourHandP3, oppHandP3, discardPileTopP3] = await Promise.all([
        player3.waitFor('YOUR_HAND'),
        player3.waitFor('OPPONENT_HAND'),
        player3.waitFor('DISCARD_PILE_TOP')
    ]);


    let prevCardOnTopOfDiscardPile: CardFace = discardPileTopP1.card;
    let player1PlayCard: Card = new Card(1, { colour: prevCardOnTopOfDiscardPile.colour, value: "4" }, { colour: Colours.Dark.Orange, value: "9" });
    ...
    let player2PlayCard: Card = new Card(2, { colour: Colours.WildCard.Black, value: ActionCards.WildCard.Measurement }, { colour: Colours.Dark.Orange, value: "8" });
    ...
    const [serverAcknowledgement,
        ...
        newNewDiscardPileTop] = await Promise.all([
            player2.waitFor('PLAYED_CARD'),
            player1.waitFor('OPPONENT_PLAYED_CARD'),
            player3.waitFor('OPPONENT_PLAYED_CARD'),
    ...
    // Assertions
    expect(serverAcknowledgement.card).toEqual(player2playedP1.card);
    expect(serverAcknowledgement.card).toEqual(player2playedP3.card);
    expect(serverAcknowledgement.playerId).toEqual(player2playedP1.opponentId);
    ...
    expect(newNewDiscardPileTop.card).toEqual(CardUtils.getActiveFace(player1PlayCard, true));
});
