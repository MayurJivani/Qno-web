...
test('Each player should receive correct hand and opponent hand', async () => {
    ...
    let [newDiscardPileTop] = await Promise.all([
        player1.waitFor('DISCARD_PILE_TOP')
    ])

    cardOnTopOfDiscardPile = newDiscardPileTop.card;
    let player2PlayCard: Card = new Card(10,{ colour: Colours.Light.Blue, value: "9" }, { colour: cardOnTopOfDiscardPile.colour, value: ActionCards.Dark.Pauli_Z });
    ...
    // Assertions
    expect(serverAcknowledgement.card).toEqual(player2playedP2.card);
    expect(serverAcknowledgement.card).toEqual(player2playedP3.card);
    expect(serverAcknowledgement.playerId).toEqual(player2playedP2.opponentId);
    ...
    expect(serverAcknowledgement.card).toEqual(newNewDiscardPileTop.card)
});
